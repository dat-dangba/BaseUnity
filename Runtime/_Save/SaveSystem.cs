...
{
    private static readonly string FILE_SAVE = "Save.bin";

    private static string file;
    private static bool loaded;
    private static DataState data;

    public static void Initialize() // initialization (used once, after the application starts)
    {
        if (!loaded)
        {
            file = FILE_SAVE;
    ...
    static void ReplaceItem(string name, string item)
    {
        ...
        for (int i = 0; i < data.items.Count; i++)
        {
            ...
            {
                data.items[i].Value = Encode(item);
                //j = true;
                //break;
                return;
            }
        ...
        //if (!j)
        data.AddItem(new SaveData(name, Encode(item)));
    }
    ...
    static Vector3 iVector3(string name, Vector3 defaultValue)
    {
        ...
        for (int i = 0; i < data.items.Count; i++)
        {
            if (string.Compare(name, data.items[i].Key) == 0)
            {
                string[] t = Decode(data.items[i].Value).Split(new char[] { '|' });
                if (t.Length == 3)
                {
    ...
    static bool iBool(string name, bool defaultValue)
    {
        for (int i = 0; i < data.items.Count; i++)
        {
            if (string.Compare(name, data.items[i].Key) == 0)
            {
                if (string.Compare(Encode(data.items[i].Value), "1") == 0) return true; else return false;
            }
        }
    ...
    static float iFloat(string name, float defaultValue)
    {
        for (int i = 0; i < data.items.Count; i++)
        {
            if (string.Compare(name, data.items[i].Key) == 0)
            {
                return floatParse(Decode(data.items[i].Value));//floatParse(Crypt(data.items[i].Value));
            }
        }
    ...
    static int iInt(string name, int defaultValue)
    {
        if (data != null)
        {
            for (int i = 0; i < data.items.Count; i++)
            {
                if (string.Compare(name, data.items[i].Key) == 0)
                {
                    return intParse(Decode(data.items[i].Value));//intParse(Crypt(data.items[i].Value));
                }
            }
        }
    ...
    static string iString(string name, string defaultValue)
    {
        for (int i = 0; i < data.items.Count; i++)
        {
            if (string.Compare(name, data.items[i].Key) == 0)
            {
                return Decode(data.items[i].Value);//Crypt(data.items[i].Value);
            }
        }
